...
export const BookModel = {
  ...
  const result = await pool.request()
    ...
    .input("BookImage", sql.NVarChar(500), data.bookImage || null)  
    .query(`
      ...
      (ISBN, BookTitle, AuthorName, PublisherName, Category, Language,
       ...
       RackLocation, PurchaseDate, PurchasePrice, VendorName, Status, BookImage)
      VALUES
      (@ISBN, @BookTitle, @AuthorName, @PublisherName, @Category, @Language,
       @Edition, @PublishYear, @TotalCopies, @AvailableCopies, @IssuedCopies,
       @RackLocation, @PurchaseDate, @PurchasePrice, @VendorName, @Status, @BookImage);
  ...
  const result = await pool.request()
    ...
    .query(`
      ...
      SET
        ...
        BookImage = COALESCE(@BookImage, BookImage),   -- ✅ Added
        UpdatedAt = GETDATE()
      WHERE BookId = @BookId;
